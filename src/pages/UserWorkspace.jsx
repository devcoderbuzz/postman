    ...
    */
    ...
    return (
        <div className="h-screen flex flex-col bg-white dark:bg-[var(--bg-primary)] text-slate-900 dark:text-[var(--text-primary)] font-sans overflow-hidden">
            ...
            {/* User Info Bar */}
            <div className="px-3 py-1 bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-xs flex justify-between items-center relative">
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <button
                            ...
                            className="font-semibold text-slate-800 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 px-2 py-0.5 rounded transition-colors flex items-center gap-1 focus:outline-none"
                        >
                            {user?.username}
                        </button>
                        {/* Dropdown */}
                        {isSettingsOpen && (
                            <>
                                ...
                                <div className="absolute top-full right-0 mt-1 w-32 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded shadow-lg z-50">
                                    <div className="py-1">
                                        <p className="px-4 py-1 text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Settings</p>
                                        <button
                                            onClick={() => { setTheme('light'); setIsSettingsOpen(false); }}
                                            className={`w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-between ${theme === 'light' ? 'text-red-600 font-medium' : ''}`}
                                        >
                                            Light Mode {theme === 'light' && <span className="text-xs">✓</span>}
                                        </button>
                                        <button
                                            onClick={() => { setTheme('dark'); setIsSettingsOpen(false); }}
                                            className={`w-full text-left px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-between ${theme === 'dark' ? 'text-red-600 font-medium' : ''}`}
                                        >
                    ...
                    <button onClick={() => { logout(); navigate('/login'); }} className="text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:underline">Logout</button>
                </div>
            </div>
                    ...
                    <>
                        <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 dark:border-[var(--border-color)] bg-slate-50 dark:bg-[var(--bg-secondary)]">
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} title="Toggle theme" className="p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400">
