...
export function BodyEditor({ bodyType, setBodyType, body, setBody, rawType = 'JSON', setRawType, environments, activeEnv }) {
    ...
    return (
        <div className="flex flex-col h-full space-y-4 overflow-hidden">
            <div className="flex items-center gap-6 px-1 border-b border-slate-200 dark:border-slate-800 pb-3 shrink-0">
            ...
            {bodyType === 'none' && (
                <div className="flex flex-col items-center justify-center flex-1 border border-dashed border-slate-200 dark:border-slate-800 rounded-lg text-slate-400 dark:text-slate-500 text-sm gap-2">
                    <p>This request does not have a body</p>
                </div>
            )}

            {(bodyType === 'raw') && (
                <div className="relative group border border-slate-200 dark:border-[var(--border-color)] rounded-lg overflow-hidden flex flex-1 bg-white dark:bg-[var(--bg-surface)]">
                    ...
                    <div className="flex-1 relative overflow-hidden h-full">
                        ...
                        <div
                            ...
                            style={{
                                color: 'transparent',
                                top: -scrollTop,
                                left: textareaRef.current ? -textareaRef.current.scrollLeft : 0,
                        ...
                        <textarea
                            ...
                            className="w-full h-full bg-transparent p-4 text-[13px] font-mono text-slate-900 dark:text-[var(--text-primary)] caret-red-500 outline-none resize-none pr-10 leading-[1.5rem] relative z-10 custom-scrollbar whitespace-pre"
                            style={{
                    ...
                    {jsonError && rawType === 'JSON' && (
                        <div className="absolute bottom-4 left-16 right-4 text-[10px] text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded px-2 py-1 flex items-center gap-2 z-20">
